...
#define	IP2MAC_TIMEOUT_SEC	1000000
	...
	for(i=0;i<Ip2Macs[deviceNo].no;i++){
		...
		if(ip2mac->addr==addr){
			...
			if(hwaddr!=NULL){
				...
				if((ip2mac->flag==FLAG_NG&&now-ip2mac->lastTime>IP2MAC_NG_TIMEOUT_SEC)){
					//FreeSendData(ip2mac);
...
char	buf[80];
	...
	//こっちに入るということはテーブルに登録されていないということ
	else{
